...
const storage = {
    ...
    saveWorkouts(data) { localStorage.setItem("workouts", JSON.stringify(data)); },
    getMeals() { return JSON.parse(localStorage.getItem("meals") || "[]"); },
    saveMeals(data) { localStorage.setItem("meals", JSON.stringify(data)); },
    getDailyInfo() { return JSON.parse(localStorage.getItem("dailyInfo") || "{}"); },
    saveDailyInfo(date, weight, bodyfat) {
        let info = storage.getDailyInfo();
        info[date] = {weight: parseFloat(weight), bodyfat: parseFloat(bodyfat)};
        localStorage.setItem("dailyInfo", JSON.stringify(info));
    },
    getInfoByDate(date) {
        const info = storage.getDailyInfo();
        return info[date] || {weight:0, bodyfat:0};
    }
};
...
function loadDashboard() {
    ...
    document.getElementById("saveInfoBtn").onclick = () => {
        ...
        if(!weight || !height || !neck || !waist) return alert("Fill all required fields!");

        let bodyfat;
        if(gender==="male"){
            bodyfat = 86.010 * Math.log10(waist - neck) - 70.041 * Math.log10(height) + 36.76;
        } else {
            if(!hips) return alert("Hips measurement required for females");
...
function loadWorkoutsPage() {
    ...
    workoutList.innerHTML = workouts.map((w,i)=>`
        <div class="card">
            ...
            <p>${w.weight} kg × ${w.reps} reps</p>
            <button class="btn" onclick="deleteWorkout(${i})">Delete</button>
        </div>
    `).join("");
...
function loadDietPage() {
    ...
    mealList.innerHTML = meals.map((m,i)=>`
        <div class="card">
            ...
            <button class="btn" onclick="deleteMeal(${i})">Delete</button>
        </div>
    `).join("");
